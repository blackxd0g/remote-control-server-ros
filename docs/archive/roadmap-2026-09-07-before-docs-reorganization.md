...
## DOC-01 — организация проектной памяти (в работе, 2026-09-07)

Согласовано: сохранить в дорожной карте цели, статусы и критерии готовности; действия и проверки перенести в отдельный Markdown-журнал, решения и ограничения закрепить в AGENTS.md и документации. Сохранить прежнюю карту в архиве. Готовность: история и цели сохранены, ссылки проверены, правила запрещают запись секретов и несанкционированную публикацию.
...
## REL-02: управление relay через Интернет без VPN — локальная разработка (2026-09-07)

- Режим по указанию пользователя: реализация и тесты только локально. Запрещены GitHub push/releases, Docker Hub push и обновление production до отдельного разрешения. Начать с индивидуальных credentials/enrollment и control gateway, затем интегрировать HBBS/HBBR, консоль и проверки. Статус готовности менять по результатам проверок, не по наличию кода.

- Запрос пользователя: убрать необходимость приватной сети для управляющих каналов. Базовый контур реализован локально; опубликованная 2.2.1 по-прежнему требует доверенной сети/VPN.
- Предлагаемый транспорт: relay сам устанавливает постоянный исходящий WSS-канал к отдельному relay-control endpoint центрального API по TCP 443. Permit/terminate/ack и telemetry/lifecycle передаются по этому каналу. Публичный UDP control 21119 не требуется; клиентские relay TCP-порты остаются доступны отдельно.
- TLS с обязательной проверкой цепочки и имени сертификата, без insecure-режима в Internet profile. Добавить Rust TLS support и доверенные CA. На центральном узле допустим существующий HTTPS reverse proxy; публиковать только предназначенный для relay endpoint, а не /internal/v1/*.
- Авторизация: индивидуальный высокоэнтропийный отзываемый credential на каждый relay, привязанный к relay ID; хранить серверный verifier, показывать credential только при выдаче. Одноразовое истекающее enrollment-разрешение, подтверждение узла администратором, ротация/отзыв, аудит. Общий internal.secret центрального сервера на relay больше не передавать. Relay credential не открывает auth snapshots, пользователей, БД или административные API.
- HBBS после собственных auth/ACL-проверок запрашивает permit через центральный API. Клиент получает relay только после подтверждения установки permit выбранным узлом; timeout/offline означает отказ или проверенный fallback. Выбор relay учитывает готовность control-канала, а не только TCP health.
- Команды ограничить по relay ID, UUID, request ID, сроку и поколению control-сессии; определить идемпотентность повторов и защиту от replay. Reconnect не должен восстанавливать отозванные/истёкшие permits. Heartbeat, backoff, таймауты, лимиты очередей/соединений и ограниченный повтор lifecycle без дублирования аудита.
- При потере control-канала существующие разрешённые пары продолжают передачу; новые permits не выдаются. Terminate без ack не показывается как успешный. Старый UDP оставить только явным legacy-режимом для доверенных сетей на время миграции, Internet profile его не запускает и не публикует.
- Этапы реализации: спецификация/негативные fixtures → серверный credential/control gateway и маршрутизация HBBS → WSS-клиент HBBR → enrollment/rotation UI → миграция Compose/RouterOS и smoke двух архитектур. Номер релиза пока не назначен.
- Приёмка: публичный HTTPS/WSS стенд без VPN, реальные выдача permit и terminate из HBBS/API, работа официального клиента через relay; неверный/просроченный TLS certificate, чужой/отозванный credential, replay, reconnect, потеря ack, недоступный API, переполнение очередей и restart дают ожидаемые безопасные результаты. Credential узла A не управляет B и не читает центральные internal API. Проверить amd64/arm64 и отсутствие UDP control listener в Internet profile.

### Локальный результат REL-02 — 2026-09-07

- Реализованы отдельный Go gateway-модуль и SQL migration для индивидуальных credentials; одноразовый enrollment, ротация через новый enrollment, отзыв, RBAC и аудит. В консоли есть выдача кода, отзыв ключа и статус control-канала.
- Добавлены Rust TLS/WSS-клиент HBBR, проверка сертификата, session/request/UUID correlation, permit/terminate/ack и telemetry/lifecycle по WSS. Общий internal.secret в WSS-режиме не читается, UDP control не запускается. Handshake имеет таймаут, переходы permit/pair/terminate синхронизированы, память очередей/permits ограничена.
- HBBS по умолчанию использует API для выбора готового relay и подтверждения permit; прямой UDP доступен только через явный legacy opt-in. Gateway не делает UDP fallback для узлов с credential-записью, в том числе отозванной.
- Проверены полный Go test/vet, Rust fmt/clippy и 28 unit tests, Vue TypeScript и Vite build. Настоящий локальный amd64 HBBR прошёл TLS-стенд: недоверенный CA отклонён, доверенная цепочка принята; TCP/TCP, WS/WS и mixed передача, отказ без permit, terminate с ack и закрытием обоих концов, запрет новых permits после отзыва, сохранение действующей пары. Конкурентный обмен enrollment успешен ровно один раз.
- Собран только локальный `remote-control-relay:rel02-dev`; запуск проверен с UID 65532, read-only и cap-drop ALL, Compose dev прошёл config validation. Это отладочная amd64-сборка с debug symbols и CA bundle, не новый версионный релиз.
- [Контракт API и проверки](relay-control-development.md); локальный relay содержит DEVELOPMENT.md, Dockerfile.dev и compose.dev.yaml. В GitHub/Docker Hub ничего не отправлено, production не изменён.
- REL-02 целиком ещё не закрыт: durable lifecycle/ack, jitter/backoff, нагрузка и сбои/restart, multi-API маршрутизация, публичный стенд, официальный клиент и ARM64 требуют отдельной локальной работы перед будущим release gate.
...
| REL-02 | P0, локальный прототип реализован; приёмка продолжается | WSS gateway, индивидуальные credentials, permit ack/replay protection, handshake timeout и лимиты; оставшиеся проверки и доработки перечислены выше |
...
- Сделать repull контейнера `rustdesk_server_routeros` на MikroTik 192.0.2.10; подтвердить версию, здоровье компонентов, сохранность данных и политику регистрации.
