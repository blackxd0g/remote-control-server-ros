...
async function downloadBundle() {
  ...
  try {
    ...
    error.value = reason instanceof Error ? reason.message : tr('Не удалось создать диагностический пакет', 'Could not create diagnostic bundle')
  } finally {
    busy.value = false
  }
}
...
<template>
  <section class="section-heading"><div><p class="eyebrow">Diagnostics</p><h2>{{ tr('Диагностика', 'Diagnostics') }}</h2><p>{{ tr('Безопасная диагностика установки для анализа неисправностей.', 'Safe installation diagnostics for troubleshooting.') }}</p></div></section>
  <p v-if="error" class="alert-error">{{ error }}</p>
  <section class="support-grid"><article class="panel support-card">
    <div class="panel-heading"><div><p class="overline">Redacted diagnostics</p><h3>{{ tr('Диагностический пакет', 'Diagnostic bundle') }}</h3></div><FileArchive :size="22" /></div>
