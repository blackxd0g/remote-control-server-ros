...
func (s *Store) UpsertConnection(ctx context.Context, value domain.ConnectionRecord) error {
	_, err := s.db.ExecContext(ctx, s.bind(`INSERT INTO connection_records
        (connection_key,actor_user_id,actor_session_id,controller_device_id,controller_name,controller_login,target_rustdesk_id,connection_type,ip,started_at,last_seen_at,closed_at)
		...
		value.TargetRustDeskID, value.ConnectionType, value.IP, millis(value.StartedAt), millis(value.LastSeenAt), nullableTimePtrMillis(value.ClosedAt))
	return err
}
...
func (s *Store) ListConnectionRecords(ctx context.Context, since time.Time, limit int) ([]domain.ConnectionRecord, error) {
	...
	for rows.Next() {
		...
		if err = rows.Scan(&value.Key, &value.ActorUserID, &value.ActorSessionID, &value.ControllerDevice, &value.ControllerName, &value.ControllerLogin, &value.TargetRustDeskID, &value.ConnectionType, &value.IP, &started, &lastSeen, &closed); err != nil {
			return nil, err
		}
